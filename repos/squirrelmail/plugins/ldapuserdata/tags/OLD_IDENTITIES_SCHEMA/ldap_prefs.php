 ...
 *
 * $Id: ldap_prefs.php,v 1.3 2004/02/10 15:02:47 avel Exp $
 *
 * @package plugins
...
function getPref($data_dir, $username, $string, $default = '') {
	...
	if(isset($_SESSION['identities'])) {
		$identities = $_SESSION['identities'];
	}
	...
	if(isset($_SESSION['ldap_prefs_cache'])) {
		...
		*/
		...
		} elseif ($string == 'full_name' || strstr($string, 'full_name') || $string == 'namepreferred') {
			/* Full name - specific identity */
			$identity_no = substr(strrchr($string, "full_name"), -1);
			
			/*
			if(!is_numeric($identity_no)) {
				$ide	
			*/

			if (isset($identities_map[$identity_no])) {
				$identity = $identities_map[$identity_no];
				if(isset($identities[$identity]['full_name'])) {
					return $identities[$identity]['full_name'];
		...
		/*
			...
			*/
			...
			/* Pref is set by user. */
			// print $string . '='. $ldap_prefs_cache[$string] . '<BR> ' ;
