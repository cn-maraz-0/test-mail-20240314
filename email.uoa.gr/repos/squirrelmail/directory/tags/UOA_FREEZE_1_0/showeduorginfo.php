 ...
 *
 * $Id: showeduorginfo.php,v 1.1.1.1 2004/01/02 16:00:18 avel Exp $
 *
 * @package plugins
...
directory_print_section_start( _("Organizational Unit Information") );
		...
		if(isset($ldq_attributes[$attr]['followme']) && isset($ldq_entry[$i][$attr])) {
			for($j=0; $j<$ldq_entry2['count']; $j++) {
				...
				if($ldq_entry2[$j]['dn'] == $askdn) {
					/* Print Link for superior */
					$attr2 = 'cn';
					$attr2_lang = $attr2.';lang-'.$ldq_lang;
					
		     			if(!is_null($ldq_entry2[$j][$attr2_lang][0]) &&
					...
					$val = directory_string_convert($ldq_entry2[$j][$attr2_lang][0], "UTF-8", $charset);
					print '<a href="showeduorginfo.php?dn='.urlencode($askdn).'">';
					print $val;
					print '</a>';
					break;
