 ...
 * @version $Id: editprofile.php,v 1.25 2006/12/18 14:26:35 avel Exp $
 * @copyright (c) 1999-2004 Alexandros Vellis <avel@noc.uoa.gr>
 */

/** Define's and includes */
...
$prev = sq_bindtextdomain ('directory_editprofile', DIR_PATH . 'locale');
...
if(isset($ldap_server[$ldq_lds]['writedn'])) {
    ...
    $ldq_write_dn = $ldap_server[$ldq_lds]['binddn'];
}

if(isset($ldap_server[$ldq_lds]['writepw'])) {
    $ldq_write_pass = $ldap_server[$ldq_lds]['writepw'];
} else {
    $ldq_write_pass = $ldap_server[$ldq_lds]['bindpw'];
}
...
foreach ($ask_attrs as $attr) {
    ...
    if(isset($ldq_attributes[$attr]['disabled']) && $ldq_attributes[$attr]['disabled'] == true) {
        continue;
    }
    ...
    if (isset($ldq_attributes[$attr]['additional_attrs']) && is_array($ldq_attributes[$attr]['additional_attrs']) ) {
...
/** Perform search! */
if (!($ldq_result = ldap_search($ldq_ldap, $ldq_base, $ldq_filter, $ldq_tattr, 0, $ldq_maxres, $ldq_timeout))) {
    echo '<p align="center"><strong>' . _("No entries found.") . '</strong></p>';
}
$entry = ldap_get_entries ($ldq_ldap, $ldq_result);
sanitize_entry_array($entry);
    ...
    foreach($ldq_editable_attrs as $attr) {
        ...
        if(isset($newinfo[$a])) {
            ...
            if ( (!isset($entry[0][$a]) && !empty($newinfo[$a])) ||
                 (isset($entry[0][$a])) && !empty($newinfo[$a]) && 
                 (!is_array($newinfo[$a]) && $newinfo[$a] != $entry[0][$a][0])
               )
            {
                //echo "<B>New / Changed Attribute: $a = ".print_r($newinfo[$a], true)."</B><BR>";
                $info[$a] = $newinfo[$a];
            }
...
if($printform == true) {
    ...
    echo '<div style="width: 60%; padding: 4px; font-weight: bold; text-align: center; margin: 1.5em auto 1.5em auto; background: yellow;">' .
        '<img style="vertical-align: middle;" alt="" src="'.SM_PATH.'plugins/famfamfam/icons/exclamation.png" /> '.
