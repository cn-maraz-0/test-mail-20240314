 ...
 * @package directory
 * @subpackage main
 * @version $Id: showeduorginfo.php,v 1.20 2007/07/05 08:37:26 avel Exp $
...
if(!empty($followme_search)) {
	foreach($followme_search as $followattr) {
		list($urlhost, $ldq_dn2) = directory_parse_eduorg_superior_url($followattr);
	}
	...
	for($i = 0; $i<$ldq_entry['count']; $i++) {
		...
		if(isset($ldq_attributes[$attr]['followme']) && isset($ldq_entry[$i][$attr])) {
			for($j=0; $j<$ldq_entry2['count']; $j++) {
		        list($urlhost, $askdn) = directory_parse_eduorg_superior_url($ldq_entry[$i][$attr][0]);

				if(isset($orgs[$askdn])) {
					/* Get from $orgs cache */
                    if($ldq_lang != 'en' && isset($orgs[$askdn]['struct;lang-'.$ldq_lang])) {
                        $askdn_desc = '<small>('. htmlspecialchars($orgs[$askdn]['struct;lang-'.$ldq_lang]) . ')</small> '. htmlspecialchars($orgs[$askdn]['text;lang-'.$ldq_lang]);
                    } else {
					    $askdn_desc = '<small>('. htmlspecialchars($orgs[$askdn]['struct']) . ')</small> '. htmlspecialchars($orgs[$askdn]['text']);
                    }
				} else {
					/* ask LDAP - FIXME */
                    //echo eduorgunit_link($askdn);
                    /*
...
print '<tr><td colspan="2" align="center">' . _("In this Organizational Unit:");

    if($directory_enable_browse) {
        echo ' <strong><a href="directory.php?searchform='.$sf.'&amp;browseorgdn='.urlencode($ldq_dn).$restofurl.'&amp;mode=2">'.
            '<img src="images/people.gif" align="center" border="0" /> ' .
            _("Browse All People") .'</a></strong> ';
    }

	echo ' <strong><a href="directory.php?searchform=yes&amp;restrict[]='.urlencode($ldq_dn).$restofurl.'&amp;mode=2"';
...
print	'><img src="images/search-16.gif" align="center" border="0" /> ' .
    ...
    if($directory_enable_browse) {
	    echo '<p>'. _("Browse people with specific affiliation:");
	...
	if($editlink > 0) {
		echo '<p><a href="editeduorginfo.php?dn='.urlencode($ldq_dn).'" '.
		'title="'._("Edit Information about this Organizational Unit") .'">'.
		'<img src="images/edit.png" alt="'._("Edit").'" align="center" border="0" />'.
		 _("Edit Information about this Organizational Unit") .
		'</a></p>';
	}
