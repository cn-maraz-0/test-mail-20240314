 ...
 * @package directory
 * @subpackage main
 * @version $Id: vcard.php,v 1.11 2006/08/01 08:56:50 avel Exp $
...
if($vcard) {
	include_once (DIR_PATH . 'include/vcard.lib.php');
	include_once (DIR_PATH . 'include/vcard_functions.php');
} else {
...
if($vcard) {
    /* FIXME this should follow the configuration options defined in config.php regarding
    ...
    SendDownloadHeaders('text', 'vcard', $filename, true);
    /*
...
if ($compose_new_win == '1') {
    ...
    // A robots.txt with Disallow: vcard.php is good, but here we also put a meta tag
    // in order to avoid being indexed.
    $xtra_head = '<meta name="googlebot" content="noindex,noarchive,nosnippet" />';

	if($ldq_standalone) {
		displayPageHeader($color, _("Personal Information"), 'no', '', $xtra_head);
	} else {
