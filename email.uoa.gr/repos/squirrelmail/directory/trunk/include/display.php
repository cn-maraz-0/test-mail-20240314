 ...
 * @package directory
 * @subpackage main
 * @version $Id: display.php,v 1.3 2006/03/12 18:02:14 avel Exp $
...
function directory_dispresultsMulti ($attributes, $entry, $sortby) {
	...
	foreach ($sorted as $key=>$i) {
		...
		foreach ($attributes as $attr) {
			...
			print '<tr><td bgcolor="'.$color[12].'" align="right" width="30%">'.
                '<small>'.$ldq_attributes[$attr]['text'].'<small></td>'.
                
                '<td bgcolor="'.$color[4].'" width="70%">';
	...
	foreach ($sorted as $key=>$i) {
		...
		if(!empty($entry[$i]['uid'][0])) {
			if($ldq_standalone) {
				...
				print '<a href="'.$vcard_uri.'&amp;vcard=1"><img src="images/icon-hcard-download.png" alt="VCard" border="0" /></a>';
			} else {
				$vcard_link = 'vcard.php?uid='.urlencode($entry[$i]['uid'][0]);
				$vcard_uri = 'plugins/directory/'.$vcard_link;
				print '<a href="'.$vcard_link.'&amp;vcard=1"><img src="images/icon-hcard-download.png" alt="VCard" border="0" /></a>';
			}
		}
		print '</td>';
		...
		if(in_array('useracl', $plugins) && !isset($popup)) {
			...
			$disp = '<img src="'.$location.'/../useracl/images/open_folder_user_add.png" border="0" ' .
...
function directory_print_attribute($attr, $entry, $privateattrs = array()) {
	global $ldq_privacy_url, $ldq_enablemailalternate, $ldq_attributes,
		$ldq_lang, $charset, $orgs, $popup, $formname, $inputname, $ldq_standalone;
	...
	if(in_array($attr, $privateattrs)) {
		...
		for ($x=0 ; $x < $entry[$attr]['count'] ; $x++) {
			switch ($attr) {
                ...
                case 'cn':
					$val = $entry[$attr][$x];
                    $vcard_uri = SM_PATH . 'plugins/directory/vcard.php?uid='.urlencode($entry['uid'][0]);
                    echo '<a href="'.$vcard_uri.'" style="text-decoration:none; color: #414042;"><strong>'.$val.'</strong></a>';
                    break;
					...
					if(isset($ldq_attributes[$attr]['followme'])) {
						...
						if(isset($follow[$val])) {
							...
							$val = directory_href($attr, $val, $uri_xtra,
							  ( isset($orgs[$val]['struct']) ? '<small>' . $orgs[$val]['struct'] . '</small> ' : '' )
							  . $orgs[$val]['text']);
						} else {
							$val = '';
						}
