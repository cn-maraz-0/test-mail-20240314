 ...
 * @package directory
 * @subpackage main
 * @version $Id: directory.php,v 1.37 2007/01/05 10:50:24 avel Exp $
...
include_once (DIR_PATH . 'include/mailto.php');
...
/* Try to find out if we are submitting a form. In IE6,
 * when pressing enter, the default submit button does not get 
 * through to $_POST like in all sane browsers. */

if((isset($_POST['search']) || isset($_POST['ldq_querystr'][0]) || isset($_POST['ldq_querystr_simple']))
   && !isset($_GET['add']) && !isset($_GET['less'])) {
...
if ($search_requested && (empty($ldq_querystr[0]) && empty($ldq_querystr_simple) ) ) {
...
if ( ($search_requested && (isset($ldq_querystr) || isset($ldq_querystr_simple) ) || isset($browseorgdn)) && !isset($error)) {
...
if(isset($restrict) && $ldq_restrict_ou && $ldq_support_eduperson){
...
/* This flag is used to mark that the produced page contains results. This is 
 * for the case that we want to hide results from search engines. */
$resultPage = true;
...
if(isset($popup)) {
	...
	if($ldq_standalone) {
        // displayPageHeader() from standalone/page_header.php has an extra argument, $xtra_head
        $xtra_head = '';
        // which we use to disable google indexing and caching of search results.
        if(isset($resultPage)) {
            $xtra_head = '<meta name="googlebot" content="noindex,noarchive,nosnippet" />';
        }
		displayPageHeader($color, _("Directory Service"), $searchform, 'onLoad="initItems();"', $xtra_head);
	} else {
		$prev = sq_bindtextdomain ('squirrelmail', SM_PATH . 'locale');
		textdomain ('squirrelmail');
...
if($searchform == 'yes') {
	...
	/** Restrict option */
	if($ldq_restrict_ou && $ldq_support_eduperson) {
 	...
 	* $titles is set and eduPerson mode is enabled. */
	
	if(isset($titles) && $ldq_support_eduperson) {
        ...
        /* Display a simpler titles list, if $titles is defined. For
         * non-eduPerson setups. */
		echo '<tr><td align="right"><strong>' . _("Title") . ':</strong></td>'.
			'<td colspan="2" align="left"><select name="ldq_title">'.
			'<option value="any"';
		if($ldq_title == _("Any"))
			echo ' selected=""';
		echo '>'. _("Any") .'</option>';
	    foreach($titles as $no => $t) {
			echo '<option';
			if(isset($ldq_title) && $ldq_title == $t[$ldq_lang]) {
				echo ' selected=""';
			}
			echo '>'.$t[$ldq_lang].'</option>';
		}
		echo '</select>' .
			'</td></tr>';
    }
	...
	foreach ($ldq_sortby_attrs as $no=>$attr) {
		echo '<option value="'.$attr.'"';
...
if(isset($ldq_entry)) {
	for ($ldq_lds=0 ; $ldq_lds < count($ldap_server) ; $ldq_lds++) {
		...
		if(isset($ldap_server[$ldq_lds]['name;lang-'.$ldq_lang])) {
			$ldq_server_description = $ldap_server[$ldq_lds]['name;lang-'.$ldq_lang];
		} else {
			$ldq_server_description = $ldap_server[$ldq_lds]['name'];
		}

		directory_print_section_start( _("Search Results") . ' - ' . $ldq_server_description);
