 ...
 * @version $Id: useracl.php,v 1.24 2007/07/05 09:36:48 avel Exp $
 *
...
if(isset($_POST['update_all']) && isset($_POST['all_users'])) {
    /* Update All */
...
if(isset($adduser) || (isset($_POST['update_all']) && isset($newuser))) {
    ...
    if(strlen($user)) {
        $new_user_perm_check = array();
        if(isset($_POST['new_user_perm_check'])) {
        ...
        if($show_ldap_cn == true) {
            if(isset($entries[$i]['cn;lang-'.$lang][0])) {
                $names[$entries[$i]['uid'][0]] =
                    useracl_string_convert($entries[$i]['cn;lang-'.$lang][0],
...
// Generate a new single token for the whole form
...
if(isset($successmsg) && $javascript_on && in_array('uoa_enhancements', $plugins)) {
echo <<<ECHO
<script language="JavaScript" type="text/javascript">
...
foreach($boxes as $no=>$box) {
    ...
    if( (isset($perm[$mbox]) && sizeof($perm[$mbox]) > 0 ) || isset($addacl)) {
        ...
        print '" method="post">';
        print '<input type="hidden" name="smtoken" value="'.$token. '" />';
...
if( ( isset($_GET['addacl']) && !isset($selected_mbox) ) ||
    ...
    if(isset($_POST['mbox'])) {
        useracl_print_addnew_separate(urldecode($_POST['mbox']));
    } else {
...
if ($compose_new_win == '1' && isset($_GET['addacl'])) {
    echo '<input type="button" name="Close" onClick="return self.close();" value="'._("Close").'"></td></tr>'."\n";
}
