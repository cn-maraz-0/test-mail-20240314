 ...
 *
 * $Id: functions.php,v 1.1.1.1 2004/01/02 16:07:15 avel Exp $
 *
 * @package plugins
...
function ldapuserdata_read_do() {
	...
	*/
	 ...
	 * cache, so that they cannot be tampered by the user. */
	// print_r($extra_data);
	...
	/*
	$dn = $extra_data['dn'];
	$identities = $extra_data['identities'];
	$identities_map = $extra_data['identities_map'];
	$alternatenames = $extra_data['alternatenames'];
	$alternateemails = $extra_data['alternatenames'];
	$authorizedreplyto = $extra_data['authorizedreplyto'];
	*/
	...
	/*
	sqsession_register($dn, 'dn');
	sqsession_register($identities, 'identities');
	sqsession_register($identities_map, 'identities_map');
	sqsession_register($alternatenames, 'alternatenames');
	sqsession_register($alternateemails, 'alternateemails');
	*/
}
...
function ldapuserdata_retrieve_data ($uid, &$user_prefs, &$extra_info) {
	global  $ldap_host, $ldap_base_dn, $ldap_bind_dn, $ldap_bind_pw,
		...
		$languages, $default_charset;

	$ldap = ldapuserdata_ldap_connect('squirrel', false);
	// $ldap = ldapuserdata_ldap_connect($uid, false);
	...
	if(isset($required_objectclass)) {
		if(!in_array($required_objectclass, $info[0]['objectclass'])) {
			$errormsg = "User $uid does not have access to the mail service.<br />";
			session_destroy();
			exit;
		}
	}
	...
	if (isset($info[0]['mailauthorizedaddress']['count'])) {
		...
		for($i=0; $i<$info[0]['mailauthorizedaddress']['count'] ; $i++) {
			...
			//$ret =  $rfc822_header->parseAddress($info[0]['mailauthorizedaddress'][$i], true );
			//print "just called:";
			//print "rfc822_header->parseAddress($info[0]['mailauthorizedaddress'][$i], true )";
			//print_r($ret);
			...
			if(strstr($info[0]['mailauthorizedaddress'][$i], "<")) {
				...
				if( ($r = preg_match($rx, $info[0]['mailauthorizedaddress'][$i], $results)) == 1) {
					...
					print "Malformed mailAuthorized Address on the LDAP server. Please contact your administrator.";
				}
			}
	...
	// $prefs['authorizedemails'][] = $info[0]['mailauthorizedaddress'][$i];
	// $prefs['alternateemails'][] = $info[0]['mailauthorizedaddress'][$i];
	// $prefs['alternateemails']['count']++;

	/* TODO: Move this out of ldap_prefs_cache */
 ...
 *
 * @todo Does not work yet; fix it.
 */
function ldapuserdata_displaychangedprefs() {
	global $color, $logoutmsg;

	// TODO - use logoutmsg facility of Squirrelmail
	// $logoutmsg = "Tralala. :-D";
