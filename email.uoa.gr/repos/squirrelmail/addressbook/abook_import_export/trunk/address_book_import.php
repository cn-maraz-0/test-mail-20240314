...
if (! sqGetGlobalVar('finish',$finish,SQ_POST)) {
    ...
    if(isset($_FILES['smusercsv'])) {
        ...
        /* ======== 2nd case - use the data that has already been found in session ======= */
        
        if (sqGetGlobalVar('csvdata',$csvdata,SQ_SESSION)) {
            sqGetGlobalVar('csvmax',$csvmax,SQ_SESSION);
            if (sqGetGlobalVar('lastimportresults',$lastimportresults,SQ_SESSION)) {
                session_unregister('lastimportresults');
            }
        } else { 
            // Problem? did not get previous data from session.
        }
    ...
    if (count($csvdata) > 0) {
        ...
        if(isset($lastimportresults)) {
            echo '<p>'. _("The entries that have not imported yet are still marked below with a checkbox. You can retry importing them to your personal addressbook, by changing the fields accordingly.") .'</p>';
        }
        ...
        while ($row < count($csvdata)) {
            ...
            echo '<input type="checkbox" name="sel' . $row . '" ';
            if(isset($lastimportresults)) {
                if(in_array($row, $lastimportresults['fail'])) {
                    echo 'checked="CHECKED"';
                }
            } else {
                // Everything checked by default, for the first time.
                echo 'checked="CHECKED"';
            }
            echo '></td>';
        ...
        sqsession_register($csvmax, 'csvmax');
    ...
    $results = array( 'fail' => array(), 'success' => array() );
    while($row < count($csvdata)) {
        if (count($csvdata[$row]) >= 5) {    // This if ensures the minimum number of columns
            $cols = count($csvdata[$row]);    // so importing can function for all 5 fields
        } else {
        ...
        if (sqGetGlobalVar($selrow,$testvar,SQ_POST)) {
            ...
            $addaddr['nickname']  = $csvdata[$row][0];
            if(empty($addaddr['nickname'])) {
                // Trying out the user part of an email address, to use for nickname,
                // since the user hasn't supplied us with one.
                if(!empty($csvdata[$row][3]) && strstr($csvdata[$row][3],'@')) {
                    $addaddr['nickname'] = substr($csvdata[$row][3], 0, strpos($csvdata[$row][3], '@') );
                }
            }
            $addaddr['firstname'] = $csvdata[$row][1];
            $addaddr['lastname']  = $csvdata[$row][2];
            $addaddr['email']     = $csvdata[$row][3];
            $addaddr['label']     = $csvdata[$row][4];
    ...
    if(empty($results['fail'])) {
        ...
        session_unregister('csvdata');
        session_unregister('csvmax');
    } else {
        // Register the last-import results so that the relevant checkboxes under "fail" will be enabled
        sqsession_register($results, 'lastimportresults');
    }
    ...
    if ($error) {
        ...
        echo '<br />'.
            '<p align="center"><a href="' . sqm_baseuri() . 'plugins/abook_import_export/address_book_import.php">' . _("Retry Importing the addresses that failed") . '</a></p>';
